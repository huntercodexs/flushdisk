...
then
	echo ""
	echo -e "${ERROR} Invalid parameter, use: ./flushdisk-remote-deploy.sh [all, {SERVICE_NAME}]"
	echo ""
	exit
fi
...
function readServiceConfiguration {
    ...
    then
        ...
        HD_TYPE=$(egrep "HD_TYPE=" "${CURRENT_FILE_CONFIG}" | cut -d "=" -f2 | sed -e "s/[^0-9a-zA-Z]//g")
    ...
    for (( i = 0; i < ${#ARRAY_SERVICES[@]}; i++ )); do
        ...
        then
            ...
            then
                ...
                echo -ne "\n${YELLOW_TEXT_COLOR}Installing${COLOR_CLOSE} ${ITEM_SERVICE} \n"
                if [[ "${SSH_USE_SUDO}" == "true" ]]
                then
                    sudo ssh ${SSH_PUB_KEY} ${SSH_USERNAME}@${SSH_HOST_ADDRESS} "mkdir -p ${FLUSHDISK_REMOTE_DIRECTORY}/backup"
                    sudo ssh ${SSH_PUB_KEY} ${SSH_USERNAME}@${SSH_HOST_ADDRESS} "mkdir -p ${FLUSHDISK_REMOTE_DIRECTORY}/services-conf"
                    sudo scp ${SSH_PUB_KEY} flushdisk.sh ${SSH_USERNAME}@${SSH_HOST_ADDRESS}:${FLUSHDISK_REMOTE_DIRECTORY}/
                    sudo scp ${SSH_PUB_KEY} flushdisk-system-colors.sh ${SSH_USERNAME}@${SSH_HOST_ADDRESS}:${FLUSHDISK_REMOTE_DIRECTORY}/
                    sudo scp ${SSH_PUB_KEY} flushdisk.conf ${SSH_USERNAME}@${SSH_HOST_ADDRESS}:${FLUSHDISK_REMOTE_DIRECTORY}/
                    sudo scp ${SSH_PUB_KEY} services-conf/${ITEM_SERVICE}.conf ${SSH_USERNAME}@${SSH_HOST_ADDRESS}:${FLUSHDISK_REMOTE_DIRECTORY}/services-conf/
                    if [[ "${HD_TYPE}" != "" ]]
                    then
                        sudo ssh ${SSH_PUB_KEY} ${SSH_USERNAME}@${SSH_HOST_ADDRESS} "sed -i 's/HD_TYPE=.*/HD_TYPE=${HD_TYPE}/g' ${FLUSHDISK_REMOTE_DIRECTORY}/flushdisk.conf"
                    fi
                else
                    ssh ${SSH_PUB_KEY} ${SSH_USERNAME}@${SSH_HOST_ADDRESS} "mkdir -p ${FLUSHDISK_REMOTE_DIRECTORY}/backup"
                    ssh ${SSH_PUB_KEY} ${SSH_USERNAME}@${SSH_HOST_ADDRESS} "mkdir -p ${FLUSHDISK_REMOTE_DIRECTORY}/services-conf"
                    scp ${SSH_PUB_KEY} flushdisk.sh ${SSH_USERNAME}@${SSH_HOST_ADDRESS}:${FLUSHDISK_REMOTE_DIRECTORY}/
                    scp ${SSH_PUB_KEY} flushdisk-system-colors.sh ${SSH_USERNAME}@${SSH_HOST_ADDRESS}:${FLUSHDISK_REMOTE_DIRECTORY}/
                    scp ${SSH_PUB_KEY} flushdisk.conf ${SSH_USERNAME}@${SSH_HOST_ADDRESS}:${FLUSHDISK_REMOTE_DIRECTORY}/
                    scp ${SSH_PUB_KEY} services-conf/${ITEM_SERVICE}.conf ${SSH_USERNAME}@${SSH_HOST_ADDRESS}:${FLUSHDISK_REMOTE_DIRECTORY}/services-conf/
                    if [[ "${HD_TYPE}" != "" ]]
                    then
                        ssh ${SSH_PUB_KEY} ${SSH_USERNAME}@${SSH_HOST_ADDRESS} "sed -i 's/HD_TYPE=.*/HD_TYPE=${HD_TYPE}/g' ${FLUSHDISK_REMOTE_DIRECTORY}/flushdisk.conf"
                    fi
                fi
                echo -ne "${OK}\n"
            fi
